...
export interface ToasterProps {
  ...
  /** Máximo de toasts visíveis ao mesmo tempo. Padrão: 3. */
  visibleToasts?: number
  /** Elemento DOM alvo do portal. Padrão: document.body. */
  container?: HTMLElement
}

export function Toaster({ position = 'bottom-right', visibleToasts = 3, container }: ToasterProps) {
  const [toasts, setToasts] = useState<ToastData[]>(() => toastStore.toasts)

  useEffect(() => {
    const unsub = toastStore.subscribe(setToasts)
    // Set.delete retorna boolean — encapsula para retornar void
    return () => { unsub() }
  }, [])
  ...
  const parts = position.split('-') as ['top' | 'bottom', 'left' | 'center' | 'right']
  const [posY, posX] = parts
  ...
  const viewport = (
    <div aria-label="Notificações" className={[styles.viewport, posClass].join(' ')}>
  ...
  return target ? createPortal(viewport, target) : viewport
}
